...
class Store extends MY_Controller {
    ...
    public function index($user_id = 0, $store_slug = null) {
        ...
        if (!empty($checkStore) && isset($checkStore['id']) && !empty($checkStore['id'])) {
            ...
            $data['category'] = $this->db->query("SELECT * FROM `categories` WHERE `parent_id` = 0 LIMIT 10")->result_array();

            $data['products'] = $this->Product_model->getProductTeBaoGoc();

            $data['productsDichVu'] = $this->Product_model->getProductDichVu();

            $data['productsDaoTao'] = $this->Product_model->getProductDaoTao();

            $data['productsPopular'] = $this->Product_model->getProductPopular();
    ...
    public function load_Product() {
        ...
        try {
            ...
            if (isset($postData['request_page'])) {
                ...
                $results_per_page = isset($postData['limit']) ? $postData['limit'] : 15;
                ...
                switch ($postData['request_page']) {
                    ...
                    case 'home':
                        ...
                        if (!isset($postData['request_page_section']) || $postData['request_page_section'] == 'new') {
                            ...
                            $new_sql .= " GROUP BY p.product_id ORDER BY p.product_created_date DESC LIMIT 15 OFFSET " . $page_first_result;
                            ...
                            if (!empty($all_categories)) {
                                ...
                                for ($i = 0; $i < sizeof($all_categories); $i++) {
                                    ...
                                    if ($all_categories[$i]['tag'] == 1) {
                                        ...
                                        $trending_sql .= " cat.mlm_categories LIKE '%hang-hoa%'";
                            ...
                            if (!empty($all_categories)) {
                                ...
                                for ($i = 0; $i < sizeof($all_categories); $i++) {
                                    ...
                                    if ($all_categories[$i]['tag'] == 1) {
                                        ...
                                        $new_sql .= " cat.mlm_categories LIKE '%hang-hoa%'";
    ...
    public function continue_last_watch() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && $userdetails['id']) {
            ...
            echo json_encode($lmsProductDetails);
        ...
        if ($product_slug) {
            ...
            if (isset($_GET['preview'])) {
                ...
                $data['product'] = $this->db->query("
					...
					SELECT *, CONCAT(users.firstname) AS product_created_by_name FROM product 
            ...
            if (!$data['product']) {
                ...
                $categoriesValue = $this->db->escape_like_str($data['categories']['0']['mlm_categories']);

                if ($categoriesValue != null) {

                    $query = "SELECT * FROM coupon WHERE categories LIKE '%$categoriesValue%'";

                    $data['coupon'] = $this->db->query($query)->result();

                    foreach ($data['coupon'] as &$coupon) {

                        $coupon->products = explode(",", $coupon->products);
                    }
                } else {
    ...
    public function add_coupon() {
        ...
        $categoriesId = $this->Product_model->getProductCategory($product_id);

        $productId = $this->Product_model->getProductPrice($product_id);



        if ($categoriesId[0]['mlm_categories'] == $coupon['categories'] && ($productId->product_price >= $coupon['total_amount'])) {
        ...
        } else if ($coupon['categories'] == null && ($productId->product_price >= $coupon['total_amount'])) {
        ...
        if (!empty($user) && !isset($json['errors']) && !isset($json['error'])) {
            ...
            if (!isset($json['errors']) && !isset($json['error'])) {
                ...
                foreach ($cart_product as $key => $product) {
                    ...
                    if ($product['refer_id'] > 0 && $refer_status) //					if($product['refer_id'] > 0)

                    {
                        ...
                        for ($l = 1; $l <= $max_level; $l++) {
                            ...
                            if ($s && $levelUser > 0) //							if( $levelUser >= 0)
    ...
    function paymentGateway($paymentGateway, $method, $uncompleted_id = '', $action = '') {
    ...
    function confirmPaymentGateway($uncompleted_id, $status, $transaction_id = '', $comment = '') {
                ...
                foreach ($products as $key => $product) {
                    ...
                    if ($product['vendor_id'])
                        ...
                        if ($product['mlm_categories'] == 'hang-hoa') {
                            ...
                            $setting = $referlevelSettings = $this->Product_model->getVendorSettings($product['vendor_id'], 'referlevel_hang_hoa');
                        } else if ($product['mlm_categories'] == 'te-bao-goc') {

                            $setting = $referlevelSettings = $this->Product_model->getVendorSettings($product['vendor_id'], 'referlevel_te_bao_goc');
                        } else if ($product['mlm_categories'] == 'dich-vu') {

                            $setting = $referlevelSettings = $this->Product_model->getVendorSettings($product['vendor_id'], 'referlevel_dich_vu');
                    ...
                    if ($product['refer_id'] > 0 && !in_array($product['refer_id'], $restricted_vendors)) {
                        ...
                        if ($product['commission'] > 0) {
                            ...
                            $comission_data_batch[] = array(
                                ...
                                'comment' => 'Hoa hồng cho Order Id order_id=' . $order_id . ' | Order By : ' . $order_info['firstname'] . " " . $order_info['lastname'] . " <br> Sale done from ip_message",
                    ...
                    if ($product['admin_commission'] > 0 && $vendors['admin_sale_status'] == 1) {
                        ...
                        $comission_data_batch[] = array(
                            ...
                            'comment' => 'Admin trả Hoa hồng Order Id order_id=' . $order_id . ' | Order By : ' . $order_info['firstname'] . " " . $order_info['lastname'] . " <br> Sale done from ip_message",
                    ...
                    if ($product['refer_id'] > 0 && !in_array($product['refer_id'], $restricted_vendors)) {
                        ...
                        if ($refer_status) {
                            ...
                            if ($product['mlm_categories'] == 'hang-goa') {

                                $level = $this->Product_model->getMyLevelHangHoa($product['refer_id']);
                            } else if ($product['mlm_categories'] == 'te-bao-goc') {

                                $level = $this->Product_model->getMyLevelTeBaoGoc($product['refer_id']);
                            } else if ($product['mlm_categories'] == 'dich-vu') {

                                $level = $this->Product_model->getMyLevelDichVu($product['refer_id']);
                            } else if ($product['mlm_categories'] == 'dao-tao') {

                                $level = $this->Product_model->getMyLevelDaoTao($product['refer_id']);
                            } else {
                            ...
                            for ($l = 1; $l <= $max_level; $l++) {
                                ...
                                if ($product['mlm_categories'] == 'hang-hoa') {
                                    ...
                                    $getSettingsFor[] = 'referlevel_' . $l . '_hang_hoa';
                                } else if ($product['mlm_categories'] == 'te-bao-goc') {

                                    $getSettingsFor[] = 'referlevel_' . $l . '_te_bao_goc';
                                } else if ($product['mlm_categories'] == 'dich-vu') {

                                    $getSettingsFor[] = 'referlevel_' . $l . '_dich_vu';
                                } else if ($product['mlm_categories'] == 'dao-tao') {

                                    $getSettingsFor[] = 'referlevel_' . $l . '_dao_tao';
                                } else {
                            ...
                            for ($l = 1; $l <= $max_level; $l++) {
                                ...
                                if ($product['mlm_categories'] == 'hang-hoa') {
                                    ...
                                    $s = $referlevelSettings['referlevel_' . $l . '_hang_hoa'];
                                } else if ($product['mlm_categories'] == 'te-bao-goc') {

                                    $s = $referlevelSettings['referlevel_' . $l . '_te_bao_goc'];
                                } else if ($product['mlm_categories'] == 'dich-vu') {

                                    $s = $referlevelSettings['referlevel_' . $l . '_dich_vu'];
                                } else if ($product['mlm_categories'] == 'dao-tao') {

                                    $s = $referlevelSettings['referlevel_' . $l . '_dao_tao'];
                                } else {
                                ...
                                if ($s && $levelUser > 0 && !in_array($levelUser, $restricted_vendors)) {
                                    ...
                                    if ($setting['sale_type'] == 'percentage') {
                                        ...
                                        $member_ship = $this->Product_model->getSettings('formsetting');

                                        $user_id = $this->Product_model->getRefid($levelUser);

                                        if ($user_id[0]['refid'])

                                            $_giveAmount = (($total * ((float)$s['sale_commition'] + $member_ship['member_ship'])) / 100);
                                    ...
                                    if ($_giveAmount > 0) {
                                        ...
                                        $comission_data_batch[] = array(
                                            ...
                                            'comment' => "Level {$l} : " . 'Hoa hồng cho Order Id order_id=' . $order_id . ' | User : ' . $order_info['firstname'] . " " . $order_info['lastname'],
                    ...
                    if ($product['vendor_commission'] > 0 && !in_array($product['vendor_id'], $restricted_vendors)) {
                        ...
                        $comission_data_batch[] = array(
                            ...
                            'comment' => 'NCC được hưởng cho order Id order_id=' . $order_id . ' | Order By : ' . $order_info['firstname'] . " " . $order_info['lastname'] . " <br> Sale done from ip_message",
    ...
    function sendOrderNoti($order_info, $products) {
        ...
        if (!isset($json) || (isset($json['errors']) && count($json['errors']) == 0)) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($checkPhone)) {
                    ...
                    $json['errors']['phone'] = "Phone Number Already Exist";
                }
    ...
    function thankyou($uncompleted_id) {
    ...
    function upload_photo($fieldname, $path) {
    ...
    function downloadable_file($filename, $mask, $order_id) {
    ...
    function order_attechment($filename, $mask) {
